...
/**
 * Pre-authenticated filter which obtains pre-authenticated user certificate subject and PEM encoded
 * certificate from request headers. Produces {@link EstEIDPrincipal} from certificate subject and
 * {@link java.security.cert.X509Certificate} as credential.
 *
 * @author Valentin Suhnjov
 */
@Slf4j
public class EstEIDRequestHeaderAuthenticationFilter
  ...
  /**
   * Try to authenticate pre-authenticated EstEID user. In case of successful authentication, filter
   * chain execution will stop resulting in status 200 response. In case of exception, exception
   * will be propagated resulting in error response.
   *
  ...
  public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
    ...
    if (requestMatcher.matches(((HttpServletRequest) req))) {
      ...
      // Filtering ends here regardless of authentication outcome. Response status indicates
   ...
   * @throws BadCredentialsException in case principal header is null or empty, or when serial
   *     number extraction fails
   */
  @Override
  ...
  /**
   * Extracts pre-authenticated credentials from request headers assuming that it is PEM encoded
   * {@link X509Certificate}.
   *
   * @param request servlet request
   * @return instance of {@link java.security.cert.X509Certificate}
   * @throws BadCredentialsException in case credentials header is null or empty, or certificate
  ...
  /**
   * Normalize certificate and retrieve actual certificate bytes. Received PEM encoded certificate
   * can have incorrect formatting caused by load balancer/proxy processing. Try to retrieve raw
   * certificate bytes given that it is encoded with base64 encoding.
   *
   * @param pem PEM encoded certificate
   * @return certificate actual bytes
   */
  private byte[] getCertificateBytes(String pem) {
    String binary = pem.replace(PEM_CERTIFICATE_HEADER, "").replace(PEM_CERTIFICATE_FOOTER, "");
