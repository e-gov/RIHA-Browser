...
public class OrganizationRoleMappingExtractor {
  ...
  private static final Pattern COMMON_NAME_VALIDATION_PATTERN =
  ...
  public OrganizationRoleMapping extract(String commonName, String displayName) {
    ...
    if (commonName == null) {
      log.debug(
          "Could not extract data from common name of organization '{}' as common name was null",
          displayName);
      return null;
    }
    ...
    organizationRoleMapping.setAuthority(
        new SimpleGrantedAuthority(ROLE_PREFIX + commonNameMatcher.group(2).toUpperCase()));
