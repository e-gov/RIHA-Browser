...
public class RihaLdapUserDetailsContextMapper extends LdapUserDetailsMapper {
  ...
  public UserDetails mapUserFromContext(
    ...
    return new RihaUserDetails(
        userDetails, ctx.getStringAttribute(UID_ATTRIBUTE), getUserOrganizationRoles(ctx));
  }

  private Multimap<RihaOrganization, GrantedAuthority> getUserOrganizationRoles(
      DirContextOperations ctx) {
      ...
      for (String groupDn : groupDns) {
        ...
        if (groupCtx != null) {
          ...
          OrganizationRoleMapping organizationRoleMapping =
              organizationRoleMappingExtractor.extract(commonName, displayName);
          if (organizationRoleMapping != null) {
            RihaOrganization rihaOrganization =
                new RihaOrganization(
                    organizationRoleMapping.getCode(), organizationRoleMapping.getName());
