...
public class SecurityContextUtil {
  ...
  /**
   * Retrieves optional {@link RihaUserDetails} from security context. User details are not present
   * when security context holds no authentication or authenticated principal is not an instance of
   * {@link RihaUserDetails}.
   *
   * @return optional of {@link RihaUserDetails}
   */
  public static Optional<RihaUserDetails> getRihaUserDetails() {
    return getUserDetails().map(userDetails -> (RihaUserDetails) userDetails);
  }

  /**
   * Retrieves optional {@link UserDetails} from security context. User details are not present when
   * security context holds no authentication or authenticated principal is not an instance of
   * {@link UserDetails}.
   *
  ...
  /**
   * Checks if current user is {@link RoleType#APPROVER} and also belongs to RIA organization with
   * code {@link #RIA_ORGANIZATION_CODE}.
   *
  ...
  /**
   * Retrieves optional {@link RihaOrganization} from security context authentication. Active
   * organization is not present when security context holds no authentication or active
   ...
   *
   * @param role role to check
   * @return true in case user is in role, false when provided role has no text or user has no such
   *     role
   */
