...
public class JsonValidationService {
  ...
  /**
   * Validates given JSON against schema. Will throw {@link JsonValidationException} if required or
   * return {@link ProcessingReport} otherwise.
   *
   * @param jsonNode validated json
  ...
  public ProcessingReport validate(JsonNode jsonNode, boolean exceptionMustBeThrown) {
    try {
      ...
      if (report.isSuccess()
          && jsonSecurityDetailsValidationService.isNecessaryToValidateSecurityDetails(jsonNode)) {
        List<ProcessingMessage> securityDetailsValidationErrorMessages =
            jsonSecurityDetailsValidationService.validate(jsonNode);
  ...
  @Autowired
  public void setJsonSecurityDetailsValidationService(
      JsonSecurityDetailsValidationService jsonSecurityDetailsValidationService) {
    this.jsonSecurityDetailsValidationService = jsonSecurityDetailsValidationService;
  }

  /**
   * Wrapper class to maintain API compatibility with old
  ...
  public static class ProcessingMessage {
    ...
    public ProcessingMessage(ValidationMessage networkntMessage) {
      ...
      try {
        ...
        ((com.fasterxml.jackson.databind.node.ObjectNode) this.jsonNode)
