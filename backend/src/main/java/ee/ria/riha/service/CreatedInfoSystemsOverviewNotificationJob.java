...
public class CreatedInfoSystemsOverviewNotificationJob {
  ...
  private static final Function<InfoSystem, InfoSystemDataModel> INFO_SYSTEM_TO_DATA_MODEL =
      infoSystem -> {
        if (infoSystem == null) {
          return null;
        }
  ...
  public CreatedInfoSystemsOverviewNotificationJob(
    ...
    to =
        Arrays.toString(
            applicationProperties.getNotification().getCreatedInfoSystemsOverview().getTo());
    Assert.notEmpty(
        Collections.singleton(to),
        "At least one recipient must be defined in the list of recipients");
  ...
  public void sendCreatedInfoSystemsOverviewNotification() {
    try {
      ...
      List<InfoSystemDataModel> infoSystemDataModels =
          infoSystems.stream().map(INFO_SYSTEM_TO_DATA_MODEL).collect(Collectors.toList());
