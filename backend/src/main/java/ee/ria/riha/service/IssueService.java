...
public class IssueService {
  ...
  public static final Function<Comment, Issue> COMMENT_TO_ISSUE =
      comment -> {
        if (comment == null) {
          return null;
        }
        ...
        return Issue.builder()
            ...
            .decisionDeadline(
                comment.getStatus() == null
                        || IssueStatus.valueOf(comment.getStatus()) == IssueStatus.OPEN
                    ? DateUtils.getDecisionDeadline(
                        comment.getCreation_date(), WORK_DAYS_UNTIL_DEADLINE)
                    : null)
            .build();
      };

  public static final Function<Issue, Comment> ISSUE_TO_COMMENT =
      issue -> {
        if (issue == null) {
          return null;
        }
  ...
  private static final Function<Comment, RihaIssueSummary> COMMENT_TO_RIHA_ISSUE_SUMMARY =
      comment -> {
        if (comment == null) {
          return null;
        }
        ...
        return RihaIssueSummary.builder()
            ...
            .title(comment.getTitle())
            .issueType(
                comment.getSub_type() != null ? IssueType.valueOf(comment.getSub_type()) : null)
            .organizationName(comment.getOrganization_name())
            .organizationCode(comment.getOrganization_code())
            .status(comment.getStatus() != null ? IssueStatus.valueOf(comment.getStatus()) : null)
  ...
  private static final Function<Comment, DashboardIssue> COMMENT_TO_DASHBOARD_ISSUE =
      comment -> {
        if (comment == null) {
          return null;
        }
        ...
        return DashboardIssue.builder()
            ...
            .organizationName(comment.getOrganization_name())
            .lastComment(
                comment.getLast_comment_id() != null
                    ? COMMENT_TO_DASHBOARD_ISSUE_COMMENT.apply(comment)
                    : null)
            .build();
      };

  private static final List<IssueType> FEEDBACK_REQUEST_ISSUE_TYPES =
      Arrays.asList(
          ESTABLISHMENT_REQUEST, TAKE_INTO_USE_REQUEST, MODIFICATION_REQUEST, FINALIZATION_REQUEST);

  private static final List<String> TOPICS_THAT_CANNOT_OPEN_FEEDBACK_REQUEST =
      Arrays.asList("x-tee alamsüsteem", "asutusesiseseks kasutamiseks", "dokumendihaldussüsteem");
  ...
  @Autowired private IssueEventService issueEventService;
   ...
   */
  public PagedResponse<Issue> listInfoSystemIssues(
      String reference, Pageable pageable, Filterable filterable) {
    InfoSystem infoSystem = infoSystemService.get(reference);

    Filterable filter =
        new FilterRequest(filterable.getFilter(), filterable.getSort(), filterable.getFields())
    ...
    return new PagedResponse<>(
        ...
        response.getContent().stream().map(COMMENT_TO_ISSUE).collect(toList()));
  }
  ...
  public PagedResponse<RihaIssueSummary> listIssues(Pageable pageable, Filterable filterable) {
    ...
    return new PagedResponse<>(
        new PageRequest(response.getPage(), response.getSize()),
        response.getTotalElements(),
        response.getContent().stream().map(COMMENT_TO_RIHA_ISSUE_SUMMARY).collect(toList()));
  }
  ...
  private void validateFeedbackRequestCanBeOpened(InfoSystem infoSystem) {
    ...
    if (infoSystem.getTopics().stream()
        ...
        .anyMatch(
            topic ->
                TOPICS_THAT_CANNOT_OPEN_FEEDBACK_REQUEST.contains(topic.trim().toLowerCase()))) {
  ...
  private void validateThereIsNoOpenFeedbackRequestIssueOfTheSameType(
      Issue model, UUID infoSystemUuid) {
    if (!isFeedbackRequestIssue(model)) {
      return;
    }
    ...
    if (!foundIssues.isEmpty()) {
      throw new ValidationException(
          "validation.issue.create.thereIsOpenFeedbackIssueOfTheSameTypeAlready");
    }
  }
  ...
  private Issue prepareIssue(Issue model) {
    RihaUserDetails rihaUserDetails =
        getRihaUserDetails()
            .orElseThrow(
                () ->
                    new IllegalBrowserStateException(
                        "User details not present in security context"));
  ...
  public Issue updateIssueStatus(Issue issue, IssueStatusUpdateModel model) {
    ...
    if (commented) {
      issueCommentService.createIssueCommentWithoutNotification(
          issue.getId(), IssueCommentModel.builder().comment(comment).build());
    }
  ...
  private void validateUpdatedFeedbackRequestIssueResolution(
      Issue issue, IssueStatusUpdateModel model) {
    ...
    if (model.getResolutionType() != IssueResolutionType.POSITIVE
        && model.getResolutionType() != IssueResolutionType.NEGATIVE) {
      throw new ValidationException(
          "validation.issue.update.unacceptableResolutionTypeForFeedbackIssue");
    }
  }

  private void closeIssue(Issue issue, IssueStatusUpdateModel model) {
    IssueResolutionType resolutionType =
        isFeedbackRequestIssue(issue) ? model.getResolutionType() : null;
  ...
  public Set<String> getParticipantsPersonalCodes(Long issueId) {
    Set<String> issueCommentsAuthorsPersonalCodes =
        issueCommentService.listByIssueId(issueId).stream()
  ...
  public void makeApprovalDecision(Issue issue, IssueApprovalDecisionModel model) {
    ...
    if (hasText(model.getComment())) {
      issueCommentService.createIssueCommentWithoutNotification(
          issue.getId(), IssueCommentModel.builder().comment(model.getComment()).build());
    }
  ...
  private void validateApprovalDecision(Issue issue, IssueApprovalDecisionModel model) {
    ...
    if (model.getDecisionType() == null) {
      throw new ValidationException(
          "validation.issueApprovalDecision.create.noDecisionTypeSpecified");
    }
  ...
  public PagedResponse<DashboardIssue> listOrganizationInfosystemIssues(
      String organizationCode, CompositeFilterRequest filter, Pageable pageable) {
  ...
  public PagedResponse<DashboardIssue> listDashboardIssues(
      Pageable pageable, CompositeFilterRequest filter) {
    boolean latestInteractionSort =
        filter.getSortParameters().contains(LATEST_INTERACTION_SORT_PARAM);
    ...
    Stream<DashboardIssue> issueStream =
        response.getContent().stream().map(COMMENT_TO_DASHBOARD_ISSUE);
  ...
  private Date latestInteraction(DashboardIssue issue) {
    ...
    return lastComment == null || dateCreated.after(lastComment.getDateCreated())
        ? dateCreated
        : lastComment.getDateCreated();
  }
}
