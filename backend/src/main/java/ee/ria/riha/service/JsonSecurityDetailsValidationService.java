...
public class JsonSecurityDetailsValidationService {
  ...
  public List<ProcessingMessage> validate(JsonNode jsonNode) {
    return Stream.of(
            validateSecurityClass(jsonNode),
            validateSecurityLevel(jsonNode),
            validateLatestAuditResolution(jsonNode))
  ...
  public ProcessingMessage validateLatestAuditResolution(JsonNode jsonNode) {
    ...
    } else if (latestAuditDate != null
        && !isValidLatestAuditResolutionStatus(latestAuditResolution)) {
  ...
  private boolean isSecurityLevelMatchingSecurityClass(String securityLevel, String securityClass) {
    ...
    int availabilityLevel =
        Character.getNumericValue(securityClass.charAt(AVAILABILITY_LEVEL_POSITION));
    int integrityLevel = Character.getNumericValue(securityClass.charAt(INTEGRITY_LEVEL_POSITION));
    int confidentialityLevel =
        Character.getNumericValue(securityClass.charAt(CONFIDENTIALITY_LEVEL_POSITION));

    int highestSecurityLevel =
        Stream.of(availabilityLevel, integrityLevel, confidentialityLevel)
            .max(Integer::compare)
    ...
    return switch (securityLevel) {
      ...
      case LOW_SECURITY_LEVEL ->
          highestSecurityLevel == LOW_SECURITY_LEVEL_CLASS_VALUE
