...
public abstract class IssueNotificationHandler<T extends SimpleHtmlEmailNotification>
    extends SimpleHtmlEmailNotificationHandler<T> {
  ...
  protected String getSubject(T notification) {
    return messageSource.getMessage(
        getSubjectKey(notification), getSubjectArgs(notification), Locale.getDefault());
  }
  ...
  protected String getText(T notification) {
    ...
    try {
      ...
      throw new NotificationHandlerException(
          "Error generating notification message text template " + templateName, e);
    }
  }
}
