...
public class FileService {
  ...
  private static final Function<FileResourceClient, FileResource>
      STORAGE_FILE_RESOURCE_TO_FILE_RESOURCE_MODEL =
          storageFileResource -> {
   ...
   */
  public UUID upload(
      InputStream inputStream, String infoSystemReference, String fileName, String contentType)
      throws IOException {
    ...
    if (!CollectionUtils.isEmpty(inputStreams) && inputStreams.size() == 2) {
      ...
      if (isVirusFound || isFileContentTypeHtml) {
        throw new IllegalBrowserStateException(
            "Faili ei saa üles laadida, kuna failitüüpi ei toetata.");
      }

      InfoSystem infoSystem = infoSystemRepository.load(infoSystemReference);

      fileUuid =
          fileRepository.upload(inputStreams.get(1), infoSystem.getUuid(), fileName, contentType);
    }
  ...
  /**
   * Downloads single file associated with info system. Checks if info system description contains
   * file link. For users which are not owners or approvers, checks if file has access restrictions.
   *
   * @param infoSystemReference info system UUID
   ...
   */
  public ResponseEntity<InputStreamResource> download(String infoSystemReference, UUID fileUuid)
      throws IOException {
   ...
   * @throws IOException in case of file repository errors
   */
  public ResponseEntity<InputStreamResource> download(InfoSystem infoSystem, UUID fileUuid)
      throws IOException {
    checkFileAccess(infoSystem, fileUuid);

    ResponseEntity<InputStreamResource> fileResource =
        fileRepository.download(fileUuid, infoSystem.getUuid());

    String attachmentContentDisposition =
        ATTACHMENT_CONTENT_DISPOSITION_TYPE
            + CONTENT_DISPOSITION_TOKEN_DELIMITER
            + getFilteredContentDisposition(
                fileResource.getHeaders().getFirst(HttpHeaders.CONTENT_DISPOSITION));
  ...
  private void checkFileAccess(InfoSystem infoSystem, UUID fileUuid) {
    List<InfoSystemFileMetadata> matchingFileMetadata =
        getMatchingFileMetadata(infoSystem, fileUuid);
    ...
    if (metadataContainsAccessRestriction(matchingFileMetadata)
        && !(hasRole(RoleType.APPROVER)
            || (hasRole(RoleType.PRODUCER)
                && infoSystemAuthorizationService.isOwner(infoSystem)))) {
  ...
  private String getFilteredContentDisposition(String contentDisposition) {
    ...
    List<String> contentDispositionTokens =
        Arrays.asList(
            StringUtils.tokenizeToStringArray(
                contentDisposition, CONTENT_DISPOSITION_TOKEN_DELIMITER));
    ...
    if (contentDispositionType.equalsIgnoreCase(INLINE_CONTENT_DISPOSITION_TYPE)
        || contentDispositionType.equalsIgnoreCase(ATTACHMENT_CONTENT_DISPOSITION_TYPE)) {
  ...
  private List<InfoSystemFileMetadata> getMatchingFileMetadata(
      InfoSystem infoSystem, UUID fileUuid) {
    return Stream.concat(
            infoSystem.getDocumentMetadata().stream(), infoSystem.getDataFileMetadata().stream())
  ...
  public PagedResponse<FileResource> list(Pageable pageable, CompositeFilterRequest filterRequest) {
    PagedGridResponse<FileResourceClient> fileResourcePagedResponse =
        fileRepository.list(filterRequest, pageable);
        ...
        .forEach(
            file -> {
              if (file.getUrl().startsWith(FILE_URL_PREFIX)) {
                ...
                String newUuid =
                    fileRepository.createFileResourceFromExisting(
                        UUID.fromString(uuid), existingInfoSystemUuid, infoSystem.getUuid());
