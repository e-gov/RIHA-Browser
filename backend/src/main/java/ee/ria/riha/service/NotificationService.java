...
public class NotificationService {
  ...
  public void sendNewIssueCommentNotification(IssueComment issueComment) {
    ...
    issueParticipantEmail.forEach(
        mail -> {
          NewIssueCommentEmailNotification notificationModel =
              createIssueCommentNotification(
  ...
  public void sendNewIssueDecisionNotification(IssueEvent decisionEvent) {
    ...
    issueParticipantEmail.forEach(
        mail -> {
          NewIssueDecisionEmailNotification notificationModel =
              createIssueCommentNotification(
  ...
  public Set<String> getIssueParticipantEmail(Long issueId) {
    ...
    Set<String> riaApproversEmails =
        userService.getApproversEmailsByOrganization(SecurityContextUtil.RIA_ORGANIZATION_CODE);
  ...
  private <T extends NewIssueCommentEmailNotification> T createIssueCommentNotification(
      T notificationModel, Long issueId, String comment, String mail) {
    InfoSystem infoSystem = infoSystemService.getByIssueId(issueId);
    Issue issue = issueService.getIssueById(issueId);

    if (mail.isEmpty()) {
      log.info(
          "New issue comment has been recently added for issue with id {}, "
              + "but none of issue participants or info system contacts have emails.",
          issueId);
      return null;
    }
    String organization =
  ...
  public void sendNewIssueToSystemContactsNotification(InfoSystem infoSystem) {
    ...
    if (sendTo.isEmpty()) {
      log.info(
          "New issue has been recently added, but info system '{}' has no contacts.",
    ...
    sendTo.forEach(
        mail -> {
          NewIssueToSystemContactsEmailNotification notificationModel =
              new NewIssueToSystemContactsEmailNotification();
          notificationModel.setFrom(getDefaultNotificationSender());
          notificationModel.setInfoSystemFullName(infoSystem.getFullName());
          notificationModel.setInfoSystemShortName(infoSystem.getShortName());
          notificationModel.setInfoSystemUuid(infoSystem.getUuid());
  ...
  public void sendIssueStatusUpdateNotification(Issue issue, boolean commented) {
    ...
    participantsEmails.forEach(
        mail -> {
          IssueStatusUpdateNotification notificationModel = new IssueStatusUpdateNotification();
          notificationModel.setFrom(getDefaultNotificationSender());
          notificationModel.setTo(mail);

          notificationModel.setIssue(
              IssueDataModel.builder()
                  .id(issue.getId())
                  .title(issue.getTitle())
                  .status(issue.getStatus().name())
                  .build());

          notificationModel.setInfoSystem(
              InfoSystemDataModel.builder()
  ...
  public void sendNewIssueToApproversNotification(Issue issue, InfoSystem infoSystem) {
    ...
    approversEmails.forEach(
        mail -> {
          NewIssueToApproversEmailNotification notificationModel =
              new NewIssueToApproversEmailNotification();
          notificationModel.setFrom(getDefaultNotificationSender());
          notificationModel.setTo(mail);
          notificationModel.setInfoSystemFullName(infoSystem.getFullName());
          notificationModel.setInfoSystemShortName(infoSystem.getShortName());
  ...
  private String[] getDefaultNotificationRecipient(String infoSystemShortName) {
    String defaultNotificationRecipientPattern =
        applicationProperties.getNotification().getRecipientPattern();
  ...
  @Autowired
  public void setEmailNotificationSenderService(
