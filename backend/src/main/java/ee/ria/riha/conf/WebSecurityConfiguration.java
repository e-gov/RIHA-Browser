...
public class WebSecurityConfiguration {
  ...
  public LdapUserDetailsService ldapUserDetailsService(
      ApplicationProperties applicationProperties, LdapContextSource contextSource) {
    LdapAuthenticationProperties ldapAuthenticationProperties =
        applicationProperties.getLdapAuthentication();
    RihaFilterBasedLdapUserSearch userSearch =
        new RihaFilterBasedLdapUserSearch(
        ...
        .oauth2Login(
            login ->
                login
                    ...
                    .redirectionEndpoint(endpoint -> endpoint.baseUri("/authenticate"))
                    .userInfoEndpoint(
                        endpoint ->
                            endpoint.oidcUserService(
                                userRequest -> {
                                  ...
                                  try {
                                    UserDetails userDetails =
                                        ldapUserDetailsService(
                                                ...
                                                contextSource(applicationProperties))
                                            .loadUserByUsername(personalCode);
                                    rihaUserDetails = (RihaUserDetails) userDetails;
                                  } catch (UsernameNotFoundException e) {
                                    // this means that the LDAP does not contain record with such
                                    // personal code
                                    rihaUserDetails =
                                        getDefaultRihaUserWithDefaultRole(personalCode);
                                  } catch (Exception e) {
                                    log.error("auth error", e);
                                    throw new OAuth2AuthenticationException(
                                        new OAuth2Error("401"), e);
                                  }
                                  rihaUserDetails.setUserRequest(userRequest);
                                  if (userRequest.getIdToken().getClaims().get("profile_attributes")
                                      instanceof Map) {
                                    ...
                                    rihaUserDetails.setFirstName(
                                        profileAttributes.get("given_name"));
                                    rihaUserDetails.setLastName(
                                        profileAttributes.get("family_name"));
                                  }

                                  // Auto-select active organization if user has only one
                                  // organization
                                  if (rihaUserDetails.getOrganizationsByCode() != null
                                      && rihaUserDetails.getOrganizationsByCode().size() == 1) {
                                    RihaOrganization singleOrganization =
                                        rihaUserDetails
                                            .getOrganizationsByCode()
                                            .values()
                                            .iterator()
                                            .next();
                                    rihaUserDetails.setActiveOrganization(singleOrganization);
                                    log.info(
                                        "Auto-selected organization {} for user {}",
                                        singleOrganization.getCode(),
                                        rihaUserDetails.getPersonalCode());
                                  }
  ...
  private CsrfTokenRepository csrfTokenRepository() {
    ...
    cookieCsrfTokenRepository.setCookieCustomizer(cookie -> cookie.httpOnly(false).path("/"));
  ...
  protected AuthenticationSuccessHandler successHandler() {
    return (request, response, authentication) -> {
      log.info(
          "Kasutaja {} ID koodiga {} logis sisse kasutades amr: {} ",
      ...
      auditLogger.log(
          ...
          request,
          ((RihaUserDetails) authentication.getPrincipal())
              .getDelegate()
              .getAuthorities()
              .toString());
  ...
  protected Filter createFromUrlSessionFilter() {
    return (request, response, chain) -> {
      if (request instanceof HttpServletRequest servletRequest
          && servletRequest.getRequestURI().contains(TARA_AUTH_ENDPOINT)) {
        String fromUrlParameter = request.getParameter(REDIRECT_URL_PARAMETER_MARKER);
        log.info(
            "authenticate request detected, fromUrl param ({}) is saved to session ",
            fromUrlParameter);
  ...
  private RihaUserDetails getDefaultRihaUserWithDefaultRole(String personalCode) {
    RihaUserDetails rihaUserDetails;
    LdapUserDetailsImpl.Essence userDetailsEssence =
        new LdapUserDetailsImpl.Essence(new DirContextAdapter());
    userDetailsEssence.setUsername(personalCode);
    userDetailsEssence.addAuthority(
  ...
  //    @Bean
  //    @Profile("!dev")
  //    ClientRegistrationRepository clientRegistrationRepository(ApplicationProperties
  // applicationProperties) {
