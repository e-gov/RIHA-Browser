...
public class RestResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  public ResponseEntity<List<JsonNode>> handleJsonValidation(JsonValidationException e) {
    ...
    return ResponseEntity.badRequest()
        .body(e.getMessages().stream().map(ProcessingMessage::asJson).collect(toList()));
  }
  ...
  public ResponseEntity<CodedBrowserExceptionModel> handleStorageClient(StorageClientException e) {
    ...
    if (storageError.getErrcode() == StorageErrorCode.INPUT_NO_OBJECT_FOUND_WITH_GIVEN_ID) {
      return ResponseEntity.status(HttpStatus.NOT_FOUND)
          .body(
              createBrowserExceptionModel(
                  e, "error.storage.objectNotFound", new Object[] {storageError.getErrtrace()}));
    }
    ...
    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
        .body(
            createBrowserExceptionModel(
                e,
                "error.storage.unhandled",
                new Object[] {
                  storageError.getErrcode(), storageError.getErrmsg(), storageError.getErrtrace()
                }));
  }

  private CodedBrowserExceptionModel createBrowserExceptionModel(
      Throwable exception, String code, Object[] args) {
  ...
  public ResponseEntity<CodedBrowserExceptionModel> handleObjectNotFound(
      ObjectNotFoundException e) {
    if (log.isTraceEnabled()) {
      log.trace("Handling object not found exception", e);
    }

    return ResponseEntity.status(HttpStatus.NOT_FOUND).body(createBrowserExceptionModel(e));
  }
  ...
  public ResponseEntity<CodedBrowserExceptionModel> handleBrowser(BrowserException e) {
    ...
    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(createBrowserExceptionModel(e));
  }
  ...
  public ResponseEntity<SizeLimitExceptionModel> handleSizeLimit(SizeLimitExceededException e) {
    ...
    return ResponseEntity.status(HttpStatus.PAYLOAD_TOO_LARGE)
        .body(
            new SizeLimitExceptionModel(
                "Faili ei saa üles laadida, kuna fail on liiga suur. Faili suurus võib olla kuni 10"
                    + " MB."));
  }
}
