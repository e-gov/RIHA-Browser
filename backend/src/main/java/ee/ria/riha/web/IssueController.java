...
public class IssueController {
  ...
  @Autowired private IssueSummaryModelMapper issueSummaryModelMapper;

  @Autowired private PrincipalRoleCheckerService principalRoleCheckerService;

  @Autowired private IssueAuthorizationService issueAuthorizationService;

  /**
   * Retrieve paginated and filtered list of issues for info system referenced by either UUID or
   * short name.
   *
  ...
  public ResponseEntity<PagedResponse<IssueSummaryModel>> listInfoSystemIssues(
      @PathVariable("reference") String reference, Pageable pageable, Filterable filterable) {
    PagedResponse<Issue> issues =
        issueService.listInfoSystemIssues(reference, pageable, filterable);

    return ResponseEntity.ok(
        new PagedResponse<>(
            new PageRequest(issues.getPage(), issues.getSize()),
            issues.getTotalElements(),
            issues.getContent().stream().map(issueSummaryModelMapper::map).collect(toList())));
  }
  ...
  @ApiPageableAndFilterableParams
  public ResponseEntity<PagedResponse<RihaIssueSummary>> listIssues(
      Pageable pageable, Filterable filterable) {
    return ResponseEntity.ok(issueService.listIssues(pageable, filterable));
  }

  /**
   * Retrieve paginated and filtered list of RIHA issues that belong to specified organization info
   * systems.
   *
  ...
  public ResponseEntity<Issue> createInfoSystemIssue(
      @PathVariable("reference") String reference, @RequestBody Issue model) {
  ...
  public ResponseEntity getInfoSystemIssue(@PathVariable("issueId") Long issueId) {
    ...
    if (principalRoleCheckerService.hasRole("ROLE_HINDAJA")
        || principalRoleCheckerService.hasRole("ROLE_KIRJELDAJA")
            && issueAuthorizationService.isIssueOwner(issueId)) {
      return ResponseEntity.ok(issueService.getIssueById(issueId));
    } else if (!(authentication.getPrincipal() instanceof RihaUserDetails)
        || ((RihaUserDetails) authentication.getPrincipal()).getAuthorities() == null) {
      HttpHeaders headers = new HttpHeaders();
      headers.setLocation(URI.create(API_V1_PREFIX + "/issues"));
      return new ResponseEntity<>(headers, HttpStatus.SEE_OTHER);
    } else {
      return ResponseEntity.status(HttpStatus.FORBIDDEN)
          .body("Teil puuduvad õigused näha seda lehekülge");
    }
  }
  ...
  public ResponseEntity<Issue> updateStatus(
      @PathVariable("issueId") Long issueId, @RequestBody IssueStatusUpdateModel model) {
  ...
  public void makeApprovalDecision(
      @PathVariable("issueId") Long issueId, @RequestBody IssueApprovalDecisionModel model) {
