...
public class InfoSystemModelMapper implements ModelMapper<InfoSystem, InfoSystemModel> {
  ...
  private static final List<String> LATEST_AUDIT_DETAILS =
      Arrays.asList("latest_audit_date", "latest_audit_resolution");
  ...
  /**
   * Maps {@link InfoSystem} to {@link InfoSystemModel} performing additional transformations that
   * depend on user authentication. Unauthenticated users will not see some json properties like
   * contacts.
   *
  ...
  public InfoSystemModel map(InfoSystem infoSystem) {
    ...
    model.setLastPositiveApprovalRequestType(
        filteredInfoSystem.getLastPositiveApprovalRequestType());
    model.setLastPositiveApprovalRequestDate(
        filteredInfoSystem.getLastPositiveApprovalRequestDate());
    model.setLastPositiveEstablishmentRequestDate(
        filteredInfoSystem.getLastPositiveEstablishmentRequestDate());
    model.setLastPositiveTakeIntoUseRequestDate(
        filteredInfoSystem.getLastPositiveTakeIntoUseRequestDate());
    model.setLastPositiveFinalizationRequestDate(
        filteredInfoSystem.getLastPositiveFinalizationRequestDate());
  ...
  private void removeLatestAuditDetailsIfUserIsNeitherInfoSystemOwnerNorApprover(
  ...
  @Autowired
  public void setInfoSystemAuthorizationService(
      InfoSystemAuthorizationService infoSystemAuthorizationService) {
