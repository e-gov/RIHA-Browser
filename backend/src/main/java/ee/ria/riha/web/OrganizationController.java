...
public class OrganizationController {
  ...
  static {
    ...
    USER_DETAILS_COMPARATORS.put(
        "firstName",
        Comparator.comparing(
            UserDetailsModel::getFirstName, Comparator.nullsLast(String::compareToIgnoreCase)));
    USER_DETAILS_COMPARATORS.put(
        "lastName",
        Comparator.comparing(
            UserDetailsModel::getLastName, Comparator.nullsLast(String::compareToIgnoreCase)));
    USER_DETAILS_COMPARATORS.put(
        "email",
        Comparator.comparing(
            UserDetailsModel::getEmail, Comparator.nullsLast(String::compareToIgnoreCase)));
    USER_DETAILS_COMPARATORS.put(
        "approver",
        Comparator.comparing(
            UserDetailsModel::getApprover, Comparator.nullsLast(Boolean::compareTo)));
    USER_DETAILS_COMPARATORS.put(
        "producer",
        Comparator.comparing(
            UserDetailsModel::getProducer, Comparator.nullsLast(Boolean::compareTo)));
  }
  ...
  public ResponseEntity<PagedResponse<UserDetailsModel>> listOrganizationUsers(
      HttpServletRequest request, Pageable pageable, CompositeFilterRequest filterRequest) {
    if (!(SecurityContextHolder.getContext().getAuthentication().getPrincipal()
        instanceof RihaUserDetails)) {
      log.error(
          "principal is not of RihaUserDetail type, but instead {}",
          SecurityContextHolder.getContext().getAuthentication().getClass().getCanonicalName());
      return ResponseEntity.badRequest().build();
    }
    RihaUserDetails rihaUserDetails =
        (RihaUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
    List<UserDetailsModel> users =
        userService.getUsersByOrganization(rihaUserDetails.getActiveOrganization().getCode());
    ...
    return ResponseEntity.ok(
        new PagedResponse<>(
            new PageRequest(pageable.getPageNumber(), pageable.getPageSize()), totalUsers, users));
  }

  private String getSortFieldFromFilterRequest(CompositeFilterRequest filterRequest) {
    return filterRequest != null
            && filterRequest.getSortParameters() != null
            && !filterRequest.getSortParameters().isEmpty()
  ...
  private List<UserDetailsModel> applyPaging(Pageable pageable, List<UserDetailsModel> users) {
    ...
    return users.subList(
        Math.min(users.size(), offset), Math.min(users.size(), offset + pageable.getPageSize()));
  }
}
