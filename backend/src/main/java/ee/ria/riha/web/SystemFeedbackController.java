...
public class SystemFeedbackController {
  ...
  public static final String RECAPTCHA_VERIFICATION_ENDPOINT =
  ...
  public ResponseEntity<Void> leaveFeedback(
      @RequestBody @Valid SystemFeedbackRequest systemFeedback, HttpServletRequest request) {

    if (applicationProperties.getFeedbackRecaptcha().isEnabled()) {

      Map<String, String> verificationRequestParams = new HashMap<>();
      verificationRequestParams.put(
          "secret", applicationProperties.getFeedbackRecaptcha().getServerKey());
      ...
      if (!responseEntity.hasBody()) {
        ...
        return reportRecaptchaResultsToClient(
            applicationProperties.getFeedbackRecaptcha().isReportErrorsToClient());
      }
      ...
      if (!recaptchaVerificationResponse.isSuccess()) {
        log.error(
            "There are problems with recaptcha integration. Verification endpoint responsed with:"
                + " {}",
      ...
      if (recaptchaVerificationResponse.getScore()
          < applicationProperties.getFeedbackRecaptcha().getPassingScore()) {
        log.debug(
            "recaptcha score did not pass for feedback: {}, recaptcha response: {}",
            systemFeedback,
            recaptchaVerificationResponse);
        return reportRecaptchaResultsToClient(
            applicationProperties.getFeedbackRecaptcha().isReportErrorsToClient());
      ...
      log.debug(
          "recaptcha response for feedback {} is {}",
          systemFeedback,
          recaptchaVerificationResponse);
    }
  ...
  private ResponseEntity<Void> sendFeedbackToBackend(
      @RequestBody @Valid SystemFeedbackRequest systemFeedback, HttpServletRequest request) {
    SystemFeedbackModel modelToSend =
        SystemFeedbackModel.builder()
