...
public class FileRepository {
  ...
  public PagedGridResponse<FileResourceClient> list(
      CompositeFilterRequest filterRequest, Pageable pageable) {
    ...
    ResponseEntity<PagedGridResponse<FileResourceClient>> responseEntity =
  ...
  /**
   * Convenience method for file resource upload without association with info system. See {@link
   * #upload(InputStream, UUID, String, String)} for details.
   *
  ...
  /**
   * Uploads file resource to RIHA-Storage and optionally associates it with info system. File
   * resource is associated with info system when info system UUID is provided.
   *
   * @param inputStream file resource input stream
   ...
   */
  public UUID upload(
      InputStream inputStream, UUID infoSystemUuid, String fileName, String contentType) {
    ...
    String response =
        restTemplate.postForObject(
            uriBuilder.toUriString(), new HttpEntity<>(parts, headers), String.class);
  ...
  private HttpEntity<InputStreamResource> createFilePart(
  ...
  /**
   * Convenience method for file resource download. See {@link #download(UUID, UUID)} for more
   * details.
   *
  ...
  /**
   * Downloads single file resource from RIHA-Storage. While info system UUID is optional and can be
   * left null, when provided will match file resource exactly by file resource UUID and info system
   * UUID.
   *
   * @param fileUuid UUID of file resource
   ...
   * @throws IOException in case of file resource streaming problems
   */
  public ResponseEntity<InputStreamResource> download(UUID fileUuid, UUID infoSystemUuid)
      throws IOException {
  ...
  public String createFileResourceFromExisting(
      UUID existingFileUuid, UUID existingInfoSystemUuid, UUID newInfoSystemUuid) {
    UriComponentsBuilder uriBuilder =
        UriComponentsBuilder.fromHttpUrl(baseUrl).path(FILE_PATH).path("/createFromExisting");
