...
public class IssueServiceTest {
  ...
  private static final UUID EXISTING_INFO_SYSTEM_UUID =
  ...
  @Mock private IssueEventService issueEventService;
  ...
  private final Issue existingIssue =
      Issue.builder().id(EXISTING_ISSUE_ID).status(IssueStatus.OPEN).build();
  ...
  public void setUp() {
    ...
    when(commentRepository.get(any(Long.class)))
        .thenAnswer(
            (Answer<Comment>)
                invocation -> {
    ...
    doNothing()
        ...
        .sendNewIssueToSystemContactsNotification(any(InfoSystem.class));
    doNothing()
        .when(notificationService)
        .sendNewIssueToApproversNotification(any(Issue.class), any(InfoSystem.class));
  }
  ...
  public void createsIssueWithTitleAndComment() {
    issueService.createInfoSystemIssue(
        EXISTING_INFO_SYSTEM_SHORT_NAME,
        Issue.builder().title("critical issue").comment("clear problem description").build());
  ...
  public void populatesAuthorAndOrganizationFromUserDetailsDuringIssueCreation() {
    issueService.createInfoSystemIssue(
        EXISTING_INFO_SYSTEM_SHORT_NAME, Issue.builder().title("title").comment("comment").build());
  ...
  public void throwsExceptionWhenActiveOrganizationIsNotSetDuringIssueCreation() {
    assertThrows(
        ...
        () -> {
          ...
          issueService.createInfoSystemIssue(
              EXISTING_INFO_SYSTEM_SHORT_NAME,
              Issue.builder().title("title").comment("comment").build());
        });
  }
  ...
  public void setsIssueTypeWhenKirjeldajaCreatesIssue() {
    Issue issue =
        issueService.createInfoSystemIssue(
            EXISTING_INFO_SYSTEM_SHORT_NAME,
            Issue.builder().title("title").comment("comment").type(ESTABLISHMENT_REQUEST).build());
  ...
  public void updatesIssueStatusWhenIssueIsClosed() {
    issueService.updateIssueStatus(
        EXISTING_ISSUE_ID, IssueStatusUpdateModel.builder().status(IssueStatus.CLOSED).build());
  ...
  public void createsClosingEventWhenIssueIsClosed() {
    ...
    verify(issueEventService)
        .createEvent(eq(EXISTING_ISSUE_ID), issueEventArgumentCaptor.capture());
  ...
  public void createsClosingIssueCommentWhenIssueIsClosed() {
    issueService.updateIssueStatus(
        EXISTING_ISSUE_ID,
        IssueStatusUpdateModel.builder()
            .status(IssueStatus.CLOSED)
            .comment("closing comment")
            .build());

    ArgumentCaptor<IssueCommentModel> issueCommentArgumentCaptor =
        ArgumentCaptor.forClass(IssueCommentModel.class);
    verify(issueCommentService)
        .createIssueCommentWithoutNotification(
            eq(EXISTING_ISSUE_ID), issueCommentArgumentCaptor.capture());
  ...
  public void throwsExceptionWhenUpdatingClosedIssue() {
    assertThrows(
        ...
        () -> {
          ...
          issueService.updateIssueStatus(
              EXISTING_ISSUE_ID,
              IssueStatusUpdateModel.builder().status(IssueStatus.CLOSED).build());
        });
  }
  ...
  public void doesNotAllowToCloseFeedbackIssueWithoutResolution() {
    assertThrows(
        ...
        () -> {
          ...
          issueService.updateIssueStatus(
              existingIssue, IssueStatusUpdateModel.builder().status(IssueStatus.CLOSED).build());
        });
  }
  ...
  public void doesNotAllowToCreateFeedbackIssueWhenThereIsOpenIssueOfTheSameType() {
    assertThrows(
        ValidationException.class,
        () -> {
          when(commentRepository.find(any()))
              .thenReturn(Collections.singletonList(existingIssueEntity));
