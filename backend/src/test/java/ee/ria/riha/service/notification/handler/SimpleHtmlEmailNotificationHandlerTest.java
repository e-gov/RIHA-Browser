...
public class SimpleHtmlEmailNotificationHandlerTest {
  ...
  @InjectMocks private TestableSimpleHtmlEmailNotificationHandler handler;
  ...
  public void setsBasicMessageParameters() throws Exception {
    ...
    verify(mimeMessage)
        .setRecipient(Message.RecipientType.TO, new InternetAddress("recipient@example.com"));
    verify(mimeMessage)
        .setRecipients(Message.RecipientType.CC, InternetAddress.parse("cc@example.com"));
    verify(mimeMessage)
        .setRecipients(Message.RecipientType.BCC, InternetAddress.parse("bcc@example.com"));
  }
  ...
  public void setsText() throws Exception {
    ...
    verify(mimeMessage)
        .setContent(ArgumentMatchers.eq("testable text"), ArgumentMatchers.eq("text/html"));
  }
  ...
  public void doesNotSetCcWhenCcIsNotDefined() throws Exception {
    ...
    verify(mimeMessage, never())
        .setRecipients(
            ArgumentMatchers.eq(Message.RecipientType.CC), ArgumentMatchers.any(Address[].class));
  }
  ...
  public void doesNotSetBccWhenBccIsNotDefined() throws Exception {
    ...
    verify(mimeMessage, never())
        .setRecipients(
            ArgumentMatchers.eq(Message.RecipientType.BCC), ArgumentMatchers.any(Address[].class));
  }

  private static class TestableSimpleHtmlEmailNotificationHandler
      extends SimpleHtmlEmailNotificationHandler {
