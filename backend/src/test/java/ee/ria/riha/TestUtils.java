...
public class TestUtils {
  ...
  public static Authentication getOAuth2LoginToken(
    ...
    OAuth2AuthorizationResponse response =
        OAuth2AuthorizationResponse.success("ok").redirectUri("redirectUri").build();
    OAuth2AuthorizationExchange authorizationRequest =
        new OAuth2AuthorizationExchange(request, response);

    Map<RihaOrganization, SimpleGrantedAuthority> organizationsMap = new HashMap<>();

    Multimap<RihaOrganization, GrantedAuthority> organizationMap =
        organizations != null
            ? organizations
          ...
          public Collection<? extends GrantedAuthority> getAuthorities() {
            return Collections.singleton(
                new SimpleGrantedAuthority(
                    RihaLdapUserDetailsContextMapper.DEFAULT_RIHA_USER_ROLE));
          }
    ...
    OAuth2User rihaUserDetails =
        new RihaUserDetails(
            userDetails, JaneAuthenticationTokenBuilder.PERSONAL_CODE, organizationMap);
    ...
    OAuth2AccessToken accessToken =
        new OAuth2AccessToken(
            OAuth2AccessToken.TokenType.BEARER, "tokenValue", Instant.now(), Instant.MAX);
  ...
  public static void setActiveOrganisation(
      Authentication authenticationToken, String organizationCode) {
