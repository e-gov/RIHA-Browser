...
public class OrganizationControllerTest {
  ...
  public void sortUsers() {
    ...
    List<UserDetailsModel> testUsers =
        Arrays.asList(
    ...
    CompositeFilterRequest filterRequest =
        new CompositeFilterRequest(Collections.emptyList(), Collections.singletonList("asc"));

    OrganizationController.sortUsers(
        testUsers, OrganizationController.USER_DETAILS_COMPARATORS.get("email"), false);
    assertThat(testUsers.get(3).getEmail(), nullValue());

    OrganizationController.sortUsers(
        testUsers, OrganizationController.USER_DETAILS_COMPARATORS.get("email"), true);
