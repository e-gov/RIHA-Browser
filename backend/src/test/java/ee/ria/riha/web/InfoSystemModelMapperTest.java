...
public class InfoSystemModelMapperTest {
  ...
  @InjectMocks private InfoSystemModelMapper infoSystemModelMapper = new InfoSystemModelMapper();
  ...
  public void doesNotMapLatestAuditDetailsIfUserIsNotAuthenticated() {
    ...
    InfoSystemModel model = infoSystemModelMapper.map(mappedInfoSystem);
    assertThat(
        model.getJson().path(SECURITY_DETAILS_KEY).path(LATEST_AUDIT_DATE_KEY).isMissingNode(),
        is(true));
    assertThat(
        model
            .getJson()
            .path(SECURITY_DETAILS_KEY)
            .path(LATEST_AUDIT_RESOLUTION_KEY)
            .isMissingNode(),
        is(true));
  }
  ...
  public void doesNotRemoveAuditDetailsIfUserIsInfoSystemOwner() {
    ...
    SecurityContextHolder.getContext()
        .setAuthentication(TestUtils.getOAuth2LoginToken(null, ACME_ORG_OWNER_CODE));
  ...
  public void doesNotRemoveAuditDetailsIfUserHasApproverRole() {
    ...
    SecurityContextHolder.getContext()
        .setAuthentication(TestUtils.getOAuth2LoginToken(organizations, ORGANIZATION_CODE));
