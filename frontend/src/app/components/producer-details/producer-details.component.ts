...
export class ProducerDetailsComponent implements OnInit, DoCheck, CanDeactivateModal {
  ...
  isBlockVisible(blockName) {
    ...
    switch (blockName) {
      ...
      case 'contacts': {
        ret =
          (this.system.hasContacts() &&
            typeof this.environmentService.getActiveUser() != 'undefined' &&
  ...
  isCannotViewCommentsErrorVisible() {
    ...
    if (this.loaded && this.userMatrix.isLoggedIn) {
      const user = this.environmentService.getActiveUser();
      return (
        this.issueId && this.userMatrix.isLoggedIn && user && !(user.canEdit(this.system.getOwnerCode()) || this.userMatrix.hasApproverRole)
      );
    } else {
      return false;
    }
  ...
  getSystem(reference) {
    this.systemsService.getSystem(reference).subscribe(
      responseSystem => {
        this.system = new System(responseSystem);
        this.generalHelperService.setRihaPageTitle(this.system.details.name);
        this.loaded = true;
        setTimeout(() => {
          this.generalHelperService.adjustSection(this.issueId ? '#tagasiside' : null);
        }, 0);
      },
      err => {
