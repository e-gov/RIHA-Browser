...
export class ProducerEditDocumentsComponent implements OnInit, CanDeactivateModal {
  ...
  addTechDoc(addForm): void {
    if (addForm.valid) {
      this.documents.push({ url: this.data.url, name: this.data.name ? this.data.name.trim() : '', type: this.data.type });
  ...
  uploadFile(event, addForm) {
    if (addForm.valid && this.docFile) {
      ...
      this.systemsService.postDataFile(this.docFile, this.system.details.short_name).subscribe(
        fileUrl => {
