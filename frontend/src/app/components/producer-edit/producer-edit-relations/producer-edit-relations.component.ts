...
export class ProducerEditRelationsComponent implements OnInit, CanDeactivateModal {
  ...
  search = (text$: Observable<string>) => {
    return text$.pipe(
      ...
      switchMap(term => (term.length < 2 ? [] : this.systemsService.getSystemsForAutocomplete(term, this.system.details.short_name))),
    );
  };

  addRelation(addForm) {
    if (addForm.valid) {
      const infoSystemShortName =
        typeof this.relation.infoSystem == 'string' ? this.relation.infoSystem : this.relation.infoSystem.details.short_name;
      this.systemsService
        .addSystemRelation(this.system.details.short_name, { infoSystemShortName: infoSystemShortName, type: this.relation.type })
        .subscribe(
          res => {
  ...
  deleteRelation(id) {
    this.systemsService.deleteSystemRelation(this.system.details.short_name, id).subscribe(
      res => {
        this.refreshRelations();
      },
      err => {
  ...
  refreshRelations() {
    this.systemsService.getSystemRelations(this.system.details.short_name).subscribe(relations => {
