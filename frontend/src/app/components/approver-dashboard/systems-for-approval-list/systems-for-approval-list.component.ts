...
export class SystemsForApprovalListComponent implements OnInit {
  ...
  private getOpenApprovalRequestsWithoutDecisions() {
    this.systemsService.getOpenApprovalRequests(this.gridData.sort).subscribe(
      res => {
        ...
        this.gridData.content.forEach(ar => {
          if (ar.events == null) {
            ...
            for (let i = 0; i < eventsCount; i++) {
              ...
              const activeOrganization = this.environmentService.getActiveUser()
                ? this.environmentService.getActiveUser().getActiveOrganization()
                : null;
              if (
                event.type == this.classifiers.event_type.DECISION.code &&
                activeOrganization &&
                activeOrganization.code == event.organizationCode
              ) {
                hasDecision = true;
                break;
              }
